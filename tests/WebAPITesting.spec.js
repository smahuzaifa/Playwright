...
const loginPayload = {userEmail:"huztest@mailinator.com",userPassword:"Test@1234"}
const orderPayLoad =  {country: "Bahamas", productOrderedId: "68a961459320a140fe1ca57a"}
//This is a JavaScript object, where the key can be passed without quotes
let loginToken; //This makes it a global variable since it is accessible outside the class/method

test.beforeAll(async()=> //Executes before all other test cases
{
    const apiContext = await request.newContext(); //Create APi testing context
    ...
    /*
    This will store the response JSON in a variable
    Here loginResponseJson is the object. In this particualtr instance it has 3 paramters, i.e
    token, userID, and message. The path to get the token is object.token.
    This can be verified with a JSON viewer application
    */
...
test.beforeEach(async()=> //Executes before each and every test cases
{
...
test ("E2E automation", async({browser}) =>
{
    ...
    //We are injecting a script that will save the token to the local storage
